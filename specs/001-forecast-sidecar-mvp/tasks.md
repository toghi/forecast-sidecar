...
- [ ] T039 [P] [US1] Integration test in `tests/integration/test_predict_smoke.py`: train a small fixture model in-test (using a stub trainer that writes `model.pkl`+`metadata.json`), then call `/forecast`; assert end-to-end correctness against the synthetic series. Add a second scenario (using `freezegun`): seed `v1`, hit `/forecast` (warm cache); promote `v2` in fake-GCS; advance time past `LATEST_POINTER_TTL_SECONDS` (60 s); hit `/forecast` again with `model_version` omitted; assert the response now resolves to `v2` without service restart (SC-009).
